...
Ortho_list <- Otis %>% 
  filter(Specialty_Description == "Orthopaedic") %>% 
  filter(DOW != 1 & DOW !=7) %>% 
  filter(!is.na(Op_start_min) & !is.na(Op_end_min))
...
In 2016 there was `r nrow(Ortho_list)` orthopaedic operations performed (including both acute & elective) where;
...
The summary of the duration os the procedure is;
...
###Lists used
```{r ortho_lists, echo=FALSE, message=FALSE, warning=FALSE}
Ortho_type <- Otis %>% 
  filter(Specialty_Description == "Orthopaedic") %>% 
  filter(AdmissionType == "AC" | AdmissionType == "WL" | AdmissionType == "AA")

Ortho_load <- Ortho_type %>% 
  group_by(Adjusted_week) %>% 
  summarize(Count = n_distinct(Anaesthetist)) %>%
  ungroup()
...
ggplot(data = Ortho_load) + 
  ...
  geom_smooth(aes(x = Adjusted_week, y = Count), colour = "red") +
  labs(title = "Total number of orthopedic lists / week", subtitle = "where a list is one or more cases performed by a single Anaesthetist in a week", x = "Weeks since 1.July.2016", y = "Number of lists") +
  theme_economist()
```

###Weekday lists used
```{r ortho_lists_day, echo=FALSE, message=FALSE, warning=FALSE}
Ortho_type <- Otis %>% 
  filter(Specialty_Description == "Orthopaedic") %>% 
  filter(DOW != 1 & DOW != 7 ) %>% 
  filter(Morning_start <= Op_start_mn & Op_start_min < Afternoon_end) %>% 
  filter(AdmissionType == "AC" | AdmissionType == "WL" | AdmissionType == "AA")
...
ggplot(data = Ortho_load) + 
  ...
  geom_smooth(aes(x = Adjusted_week, y = Count), colour = "magenta") +
  labs(title = "Total number of week day & working hours orthopedic lists / week", subtitle = "where a list is one or more cases performed by a single Anaesthetist in a week", x = "Weeks since 1.July.2016", y = "Number of lists") +
  theme_economist()
```

###Time series
```{r ortho_ts, echo=FALSE, warning=FALSE, message=FALSE}
Ortho_type <- Otis %>% 
  filter(Specialty_Description == "Orthopaedic") %>% 
  filter(AdmissionType == "AC" | AdmissionType == "WL" | AdmissionType == "AA")

Ortho_load <- Ortho_type %>% 
  group_by(Adjusted_week, AdmissionType) %>% 
  summarize(Count = n()) %>%
  ungroup()
...
ggplot(data = Ortho_load) +
  ...
  labs(title = "Orthopaedic cases Jan.2016-Apr.2017", x = "Weeks", y = "Number of cases / week") +
...
### WL vs AC
```{r othero_model, echo=FALSE, warning=FALSE, message=FALSE }
ggplot(data = Ortho_load) +
  geom_point(aes(x = WL, y = AC), colour = "tomato") +
